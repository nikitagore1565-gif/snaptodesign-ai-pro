...
async function anthropic(body){
  if(!AI_KEY) throw new Error('ANTHROPIC_API_KEY is not configured on the server');
  const r=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'content-type':'application/json','x-api-key':AI_KEY,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:body.model||AI_MODEL,max_tokens:body.max_tokens||4000,system:body.system||'',messages:body.messages||[]})});
  const d=await r.json().catch(()=>({})); if(!r.ok) throw new Error(d?.error?.message||`Anthropic HTTP ${r.status}`);
  return {text:(d.content||[]).filter(x=>x.type==='text').map(x=>x.text).join('\n').trim(),usage:d.usage||{}};
}
async function route(req,res){
  ...
  try{
    ...
    if(req.method==='POST'&&p==='/api/ai'){
      const x=requireUser(req,res);if(!x)return;if(!consumeCredit(x.id,1))return json(res,402,{error:'Credits exhausted',credits:creditFor(x.id)});const b=await readBody(req);try{const out=await anthropic(b);return json(res,200,{text:out.text,usage:out.usage,credits:creditFor(x.id)});}catch(e){const c=creditFor(x.id);c.balance++;c.used=Math.max(0,c.used-1);saveDB();return json(res,502,{error:e.message,credits:c});}
    }
